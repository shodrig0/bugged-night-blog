      ...
      <Slider {...settings}>
        {news.map((item) => (
          <div key={item.id} className="card overflow-hidden flex">
            <div className="relative h-96 md:h-[500px] overflow-hidden flex w-full">
              {/* Imagen */}
              ...
              <div className="absolute inset-0 flex flex-col justify-between p-8">
                {/* Bloque superior (categoría, título, fecha, contenido) */}
                <div>
                  ...
                  {item.metadata?.publication_date && (
                    <p className="text-gray-400 mb-4">
                      {format(
                        new Date(item.metadata.publication_date),
                        "MMMM dd, yyyy"
                      )}
                  ...
                  <div
                    className="text-gray-300 line-clamp-3 max-w-2xl"
                ...
                {/* Botón abajo separado */}
                {item.metadata?.related_link && (
                  <a
                    href={item.metadata.related_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-4 inline-block btn btn-primary self-start"
                  >
                    Read More
                  </a>
                )}
